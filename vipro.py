...
vipro = LINETCR.LINE()
#vipro.login(qr=true)
vipro.login(token="EqoTYNqti7FxFjplxMu7.MrqqXYdtjPuQUAA29Stb1W.fRy+SQU2YrPeE68DiPF5Ldlv33Wl+XY0SDgR1wYpqaw=")
vipro.loginResult()
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                     contact = vipro.getContact(msg.from_)
                     cName = contact.displayName
                     balas = ["ada apa sappo??!! nantipi na.Lagi Sibuk deng mulik",cName + " pm aja cappo kalo ga dibalas deng mulik",cName + " lagu anu deng mulik, pm aja klo nda dibalas na 😊😊", cName + " dekgaga deng mulik, ko engka penting pm bani sappo na 😊😊 " + cName, "matindro deng mulik matupi nabacai🙏🙏🙏 " + cName, "iyek sappo..😙😙 " + cName + "?", "Ada Perlu Apa sappoku " + cName + "?","Woii " + cName + " agatu?? hahaha"]
                     ret_ = random.choice(balas)
                     name = re.findall(r'@(\w+)', msg.text)
                     mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                     mentionees = mention['MENTIONEES']
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention2"] == True:          
                    contact = vipro.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["sabar ya sappo..lagi anu deng mulik😆😆 " + cName + " dekgagai deng mulik!"]
                    ret_ = random.choice(balas)
                    name = re.findall(r'@(\w+)', msg.text)
                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                    mentionees = mention['MENTIONEES']
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention3"] == True:          
                    contact = vipro.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["Woii " + cName + ", 😨😨😨 sabar om, deng mulik lagi sibuk!"]
                    balas1 = "yg ngetag ..yg cantik n ganteng. gw pampangin potonya 😆😆😆. ."
                    ret_ = random.choice(balas)
                    image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus
                    name = re.findall(r'@(\w+)', msg.text)
                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
